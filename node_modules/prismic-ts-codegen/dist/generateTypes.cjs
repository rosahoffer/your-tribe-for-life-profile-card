...
const index = require('./_node_modules/quick-lru/index.cjs');
const addLine = require("./lib/addLine.cjs");
const addSection = require("./lib/addSection.cjs");
const buildCustomTypeType = require("./lib/buildCustomTypeType.cjs");
const buildSharedSliceType = require("./lib/buildSharedSliceType.cjs");
const buildUnion = require("./lib/buildUnion.cjs");
const cache = new index({ maxSize: 1e3 });
function generateTypes(config = {}) {
  ...
  code = addLine.addLine(`import type * as prismic from "${typesProvider}";`, code);
  if (((_a = config.clientIntegration) == null ? void 0 : _a.includeCreateClientInterface) || ((_b = config.clientIntegration) == null ? void 0 : _b.includeContentNamespace)) {
    if (typesProvider !== "@prismicio/client") {
      ...
      code = addLine.addLine(`import type * as ${clientImportName} from "@prismicio/client";`, code);
    }
  }
  code = addSection.addSection(`type Simplify<T> = { [KeyType in keyof T]: T[KeyType] };`, code);
  const contentTypeNames = [];
  if (config.customTypeModels) {
    const allDocumentTypesTypeNames = [];
    for (const model of config.customTypeModels) {
      const customTypeType = buildCustomTypeType.buildCustomTypeType({
        model,
      ...
      for (const auxiliaryType of customTypeType.auxiliaryTypes) {
        code = addSection.addSection(auxiliaryType.code, code);
      }
      code = addSection.addSection(customTypeType.code, code);
    ...
    if (config.customTypeModels.length > 0) {
      ...
      const allDocumentTypesUnion = buildUnion.buildUnion(allDocumentTypesTypeNames);
      code = addSection.addSection(`export type ${allDocumentTypesUnionName} = ${allDocumentTypesUnion};`, code);
      contentTypeNames.push(allDocumentTypesUnionName);
    }
  }
  if (config.sharedSliceModels) {
    for (const model of config.sharedSliceModels) {
      const sharedSliceType = buildSharedSliceType.buildSharedSliceType({
        model,
        fieldConfigs,
        cache: shouldUseCache ? cache : void 0
      });
      code = addSection.addSection(sharedSliceType.code, code);
  ...
  if (((_c = config.clientIntegration) == null ? void 0 : _c.includeCreateClientInterface) || ((_d = config.clientIntegration) == null ? void 0 : _d.includeContentNamespace)) {
    ...
    if (config.clientIntegration.includeCreateClientInterface) {
      if ((((_e = config.customTypeModels) == null ? void 0 : _e.length) || 0) > 0) {
        clientModuleCode = addSection.addSection(`interface CreateClient {
    ...
    if (config.clientIntegration.includeContentNamespace) {
      clientModuleCode = addSection.addSection(commonTags.source`
					namespace Content {
						export type {
