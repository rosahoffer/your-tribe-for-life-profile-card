...
const _1 = require(".");
function randomBlock(fieldsDefs, mockConfigs) {
    ...
    return {
        ...
        primary: (() => {
            if (!variationDef.primary)
                return {};
            return randomBlock(Object.entries(variationDef.primary), config === null || config === void 0 ? void 0 : config.primaryFields).reduce((acc, [k, v]) => ({ ...acc, [k]: v }), {});
        })(),
        items: (() => {
            ...
            return Array(nbItemsBlocks)
                .fill(null)
                .map(() => {
                return randomBlock(Object.entries(variationDef.items || {}), config === null || config === void 0 ? void 0 : config.itemFields);
            })
                .map((b) => ({
...
function patchWidgets(diff, content, config) {
    const patched = Object.entries(diff).reduce((acc, [widgetId, widgetChange]) => {
        ...
        const patched = _1.NestableWidgetMock.patch(widgetChange, widgetContent, widgetConfig);
        return {
            ...acc,
