...
const images = dataset as ReadonlyArray<
	{
		raw: string
		userName: string
		orientation: string
...
function generateImageView(img: {
	...
	const [originUrl, originWidth, originHeight] = (() => {
		...
		const generatedUrl = randomImage?.raw.split("?")[0]
		if (!generatedUrl)
...
export function random(def: Image): ImageContent {
...
export const ImageMock: NestableMock<Image, ImageContent> = {
	generate(def: Image): ImageContent {
		return random(def)
	},
