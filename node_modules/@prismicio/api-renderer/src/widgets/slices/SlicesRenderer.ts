...
function renderContent(
	slice: SlicesContentItem,
	sliceFetch: GroupOrField | Slice | undefined,
	ctx: RenderContext,
	keyToSliceDef: Record<string, SliceDefO>,
): O.Option<unknown> {
	if ("__TYPE__" in slice.widget && slice.widget.__TYPE__ === EmptyContentType) {
		return O.none
	} else if (!(slice.name in keyToSliceDef)) {
		return O.none
	} else {
		return pipe(
			SliceWidgetRenderer(ctx).renderV2(
				keyToSliceDef[slice.name] as SliceDefO,
				slice.widget,
				sliceFetch,
			) as O.Option<unknown>,
			O.map((render) => ({
...
const SlicesRenderer: (ctx: RenderContext) => Renderer<SlicesDefO, SlicesContent, SliceZone> = (ctx) => ({
	...
	renderDefault(_def: SlicesDefO): unknown {
		return []
	},
